...
SpawnEntity :: proc(_world : ^ecs.Entity_World, _viewport_w : f32, _velocitySpeed : f32) {
    ...
    ecs.AddComponentToEntityWorld(_world, &_world.transforms, e,
        components.Transform{pos = {rand.float32_range(0, _viewport_w), rand.float32_range(0, 10)}, rot = 0}, .Transform)

    ecs.AddComponentToEntityWorld(_world, &_world.velocities, e,
        components.Velocity{linear = {rand.float32_range(-10, 10), rand.float32_range(10, 30) * _velocitySpeed}}, .Velocity)
